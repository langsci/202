...
constructions which include, among others, compounds (\textnl{hot
  dog}), \isi{verb-particle constructions} (e.g., \textnl{take off}), complex
prepositions (e.g., \textnl{on top of}), adverbials (e.g., \textnl{by and large})
and verbal phrases (e.g., \textnl{spill the beans}). MWEs are characterized
...
form a verb \isi{phrase}.\footnote{The internal structure of some \isi{idioms} can be syntactically idiosyncratic (e.g., \textnl{find fault}, \textnl{close up shop}).}
...
(``idiomatic phrases''). The former are \isi{idioms} whose meaning, once known, can be distributed among their parts. A typical example is \textnl{spill the
...
\isi{passivization}.  \citet{schenk95} showed that non-decomposable \isi{idioms} in
...
Idiomaticity is often characterized by {\scshape conventionality}; the meaning of \isi{idioms} cannot be entirely predicted from the meaning of their parts when they appear in isolation from one another. There are, however, a number of other semantic dimensions according to which \isi{idioms} can be characterized. The dimension which \citet[p. 498]{nunberg94} assume plays a crucial role in determining the behavior of an idiom is its \isi{decomposability}. Nevertheless, as was previously mentioned, determining whether an idiom is decomposable or not is rather impressionistic, and is prone to circularity, where its \isi{flexibility} is taken as evidence for its \isi{decomposability}.

In this chapter we consider an alternative categorization of \isi{idioms}. More precisely, we cross-classify \isi{idioms} according to two dimensions: {\scshape figuration} and {\scshape transparency}. Figuration reflects the degree to which the idiom can be assigned a literal meaning. Transparency (or opacity) relates to how easy it is to recover the motivation for an idiom's use, or, in other words, to explain the relationship between its literal meaning and its idiomatic one. Idioms are {\scshape figurative} if their literal meaning can conjure up a vivid picture in the speaker's mind. Within the figurative \isi{idioms} we distinguish between two types. In {\scshape transparent figurative} \isi{idioms} the relationship between the literal picture and the idiomatic meaning is perceived to be motivated. \ili{English} examples include \textnl{saw logs} (`snore') and \textnl{the cat's out of the bag} (`previously hidden facts were revealed'). Conversely, {\scshape opaque figurative} \isi{idioms} portray a picture whose relationship to the idiomatic meaning is not perceptible. \ili{English} examples include \textnl{shoot the breeze} (`chat') and \textnl{chew the fat} (`talk socially, gossip'). Idioms which are not figurative do not have a comprehensible literal meaning, and as such are necessarily opaque. Among these \isi{idioms} we find what are referred to as ``cranberry \isi{idioms}'' \citep{moon98,trawinskisailersoehnetal.2008}, which, similarly to ``cranberry morphemes'', have parts which have no meanings (e.g., \textnl{run amok} `behave in an unrestrained manner' and \textnl{take umbrage} `take offense'). These \isi{idioms} may have been figurative and transparent once, but synchronically they contain a word whose meaning is not accessible to contemporary speakers.\footnote{Opaque non-figurative \isi{idioms} are not necessarily cranberry \isi{idioms}. One \ili{Hebrew} example is \idgloss{natan ba-kos {\ayin}ein-o}{gave in the cup his eye}{got drunk}. Although all the words in this idiom are common ``everyday'' words, it does not conjure up any type of image. Such \isi{idioms} seem to be rare.}
...
\noindent\idgloss{yarad me-ha-{\ayin}ec}{descended from the tree}{conceded}
...
\noindent\idgloss{hosif {\shin}emen la-medura}{added oil to the bonfire}{aggravated the situation} This metaphorical idiom describes the act of making a situation worse than it already is. A similar \ili{English} idiom is \textnl{add fuel to the fire}.
...
\noindent\idgloss{{\tet}aman yad-o ba-cala{\het}at}{buried his hand in the plate}{refrained from acting}
...
\noindent\idgloss{he{\ayin}ela {\het}eres \textnormal{(}be-yad-o\textnormal{)}}{brought up a shard \textnormal{(}in his hand\textnormal{)}}{tried in vain, failed}
...
\noindent\idgloss{yaca me-ha-kelim}{came out from the tools}{became upset}
...
\noindent\idgloss{{\alef}avad {\ayin}al-av \textnormal{(}ha-\textnormal{)}kela{\het}}{\textnormal{(}the-\textnormal{)}\textsc{kelah} was lost on him}{became outdated} The cranberry word in this idiom is \textnl{kela{\het}}, which has no known literal meaning. It appears three times in the Old Testament, twice as a name of a place, and once as part of this idiom (Book of Job 30:2). Nevertheless, this idiom is an established part of the \ili{Hebrew} lexicon. Interestingly, although the noun \textnl{kela{\het}} is indefinite in the original Biblical idiom, in Modern \ili{Hebrew} it is mostly used with a definite prefix.
...
\noindent\idgloss{higdi{\shin} {\alef}et ha-se{\alef}a}{overfilled the \textsc{seah}}{exaggerated} The word \textnl{se{\alef}a} is originally a biblical unit of measurement, usually of grain, but it is rarely used outside of this idiom (exceptions are texts which deal with religious laws). Interestingly, the verb \textnl{higdi{\shin}} is hardly used outside of this context as well, although the consonantal root g-d-{\shin} is productive in a different verbal \isi{template} (\hebgloss{gada{\shin}}{fill}).\footnote{Semitic morphology is largely based on roots-and-patterns. Roots are sequences of (typically) three consonants. Patterns are sequences of vowels and possibly consonants with open slots for the roots consonants, indicated by capital Cs. For example, \textnl{higdi{\shin}} and \textnl{gada{\shin}} are formed by combining the same consonantal root  g-d-{\shin} with two different templates: hiCCiC and CaCaC.} The original literal meaning of the verb \textnl{higdi{\shin}} was `to gather wheat sheaves', and the literal meaning of the \isi{phrase} was to overfill a set measure with wheat. The idiom can be used with an agentive subject \reff{she:seah-canon1} who ``overdoes it'', or an abstract noun \reff{she:seah-canon2} which in itself is ``too much''.
...
\noindent\idgloss{lo yesula be-paz}{will not be \textsc{sula} in gold}{priceless} Unlike the other \isi{idioms} in this category, the cranberry word in this case is a verb: \textnl{sula}. The verb is formed in a passive morphological \isi{template} (CuCaC) and is never used in the active \isi{template} (CiCeC). Its original (Biblical) meaning is `was measured', but it is not used out of this context in \ili{Hebrew}. The noun \hebgloss{paz}{gold} is a very rare synonym of the commonplace \textnl{zahav}; its distribution is mostly restricted to fixed phrases (e.g., \hebgloss{hizdamnut paz}{golden opportunity}) and the current idiom.
...
In addition, we include in the category of syntactic variation two types of \isi{long-distance dependencies}: topicalization/focalization and \isi{relativization}. Information structure considerations motivate the fronting of VP-internal material to a clause-initial position. A fronted constituent can be a focal element \reff{she:ex:focus} or a topicalized element \reff{she:ex:topic}.
...
An additional long-distance dependency involves \isi{relativization}. When NP complements are relativized in \ili{Hebrew} a resumptive pronoun can optionally occur in the \isi{relativization} site \reff{she:ex:rel-np}. Oblique complements are obligatorily resumed by a pronoun, and the language does not allow preposition stranding \reff{she:ex:rel-pp}.
...
Verb second configurations are found with all types of \isi{idioms}. In what follows are examples of \textit{transparent figurative idioms} (\ref{she:ec-v2}--\ref{she:medura-v2}), \textit{opaque figurative idioms} (\ref{she:xeres-v2}--\ref{she:yaca-v2}), and \textit{cranberry idioms} (\ref{she:meduxa-v2}--\ref{she:sea-v2}). Note that in all these examples, the clause-initial ``trigger'' is not part of the idiom. The fronting of idiom parts is discussed in Section \ref{she:sec:topicalization}.
...
Instances of fronting of idiom parts were found across the three semantics types. Following are examples of \textit{transparent figurative idioms} (\ref{she:ec-top} - \ref{she:medura-top}), \textit{opaque figurative idioms} \reff{she:taman-top}, and \textit{cranberry idioms} (\ref{she:meduxa-top} - \ref{she:yesula-top}). Note that the fronted part is within square brackets.
...
As is evident from these examples, fronted idiom parts do not usually appear in their canonical form. The fronted constituents in \reff{she:ec-top} and \reff{she:taman-top} are modified with the focal marker \hebgloss{gam}{also}. In \reff{she:ec-top} the demonstrative \hebgloss{ha-ze}{the-this} further emphasizes the contrastive interpretation. The topicalized idiom parts in \reff{she:medura-top} and \reff{she:meduxa-top} include modifiers. In fact, in \reff{she:medura-top} both idiomatic complements are fronted. In examples \reff{she:meduxa-top} and \reff{she:sea-top} the topicalized elements are cranberry words. In \reff{she:meduxa-top} the modifier \hebgloss{{\het}a{\shin}uva}{important} reveals that \textnl{meduxa} is interpreted in this context as the issue on which the parents are deliberating (see more about this in Sections \ref{she:sec:li} and \ref{she:sec:ls}).

\subsubsection{Relativization}
\label{she:sec:relativization}
When idiom parts are relativized, they surface as the head of the relative clause, which is modified by it. Instances of \isi{relativization} were found for \textit{transparent figurative idioms} and \textit{cranberry idioms}. Examples are given in (\ref{she:medura-rc} - \ref{she:ec-rc}) and (\ref{she:meduxa-rc} - \ref{she:kelax-rc}), respectively, with the relative clause in square brackets. No instances of \isi{relativization} were found for \textit{opaque figurative idioms}.
...
The cranberry words in these examples are clearly functioning outside their idiomatic context. In \reff{she:meduxa-rc} the cranberry word \textnl{meduxa} serves as the complement of the verb \hebgloss{heqimu}{established} in a unique and innovative yet comprehensible combination. In \reff{she:kelax-rc} \textnl{kela{\het}} is the head of a relative clause, yet it also functions as a subject of predicate: \hebgloss{kvar hi{\het}lid mizman}{rusted long ago}. The speaker in this case attributes to this cranberry word physical properties which are related to aging. In doing so, s/he emphasizes his/her assessment of the bureau as an old and outdated institution.
...
Variations with respect to the mapping of idiom parts to grammatical functions were found with \textit{transparent figurative idioms}. Consider examples \reff{she:medura-passive} and \reff{she:medura-rev}. In these examples \hebgloss{{\shin}emen}{oil}, which is the complement of the verb in the canonical idiom, functions as the subject.
...
In the \textit{cranberry idiom} \idgloss{higdi{\shin} {\alef}et ha-se{\alef}a}{overfilled the \textsc{seah}}{exaggerated} the cranberry word \textnl{ha-se{\alef}a} functions as the complement of the verb.  There are, however, instances of this idiom where \textnl{ha-se{\alef}a} functions as the subject and the verb is a morphological variant of the canonical verb: the passive \hebgloss{hugde{\shin}a}{was.overfilled} in \reff{she:sea-passive} and the middle \hebgloss{nitgad{\shin}a}{was.overfilled} in \reff{she:sea-arg-rev}.
...
The verbs \hebgloss{yarad}{descend}, \hebgloss{yaca}{come out} and  \hebgloss{ya{\shin}av}{sit}, which head a \textit{transparent figurative}, \textit{opaque figurative} and \textit{cranberry idiom}, respectively, are also used in their causative form in the same \isi{idioms}. The causee argument, indicated by square brackets, is an ``open slot''. This is illustrated in \reff{she:ec-cause}, \reff{she:yaca-cause} and \reff{she:meduxa-cause}.
...
The following example exhibits a neologism. The verb \textnl{he{\alef}evid} is created by combining the consonantal root of the original verb \hebgloss{{\alef}avad}{lose} ({\alef}-b/v-d) with the causative \isi{template} HiCCiC to create a verb whose meaning is `cause to be lost'.\footnote{The original Biblical meaning of \textnl{he{\alef}evid} is `demolish, destroy'.} The cranberry word \textnl{kela{\het}} serves as the causee and surfaces as a direct object (marked with the accusative case marker \textnl{{\alef}et}). This suggests that it is interpreted (at least in this case) as some property, perhaps relevance, whose absence makes something outdated. Although this neologism is attested only once in the corpus, it is comprehensible in the context of this idiom, due to the transparent morphological relationship between it and the canonical verbal form.
...
\textit{Transparent figurative idioms} were found to be amenable to different types of modifications. Consider the following examples with the idiom \idgloss{yarad me-ha-{\ayin}ec}{descended from the tree}{conceded}.
...
The occurrence of modification in \textit{cranberry idioms} is especially surprising due to their opaqueness and lack of \isi{figuration}. Nevertheless, as the following examples show, cranberry words are compatible with different types of modifications. In \reff{she:meduxa-ins-int-mod} the cranberry word \textnl{meduxa} is modified by an adjectival \isi{phrase} and with a demonstrative, in \reff{she:meduxa-rc-insertion} by a relative clause and in \reff{she:meduxa-cs-insertion} \textnl{meduxa} is the head of a construct state NP which is modified by its complement. In all three instances, the modification suggests that the speakers perceive the interpretation of \textnl{meduxa} to be `issue'. A similar case was presented in \reff{she:meduxa-top} above.
...
Instances of lexical insertions with the other cranberry \isi{idioms} were also found. In \reff{she:kelax-ins-int-mod-quant} the cranberry word \textnl{kela{\het}} appears with an indefinite quantifier and an adjective, although it is not clear what it denotes, neither literally nor idiomatically. In \reff{she:sea-rc-insertion} the cranberry word \textnl{se{\alef}a} is modified with a relative clause, which refers to its literal meaning as a measure.
...
Note that \textit{transparent figurative idioms} exhibit lexical substitution of both verbs and nouns. In \reff{she:ec-sub-verb} the verb \hebgloss{yarad}{descend} is replaced with the more active \hebgloss{qafac}{jump}, without loss of idiomatic meaning.
...
\textit{Cranberry idioms} are also subject to lexical substitutions. There are a few instances of the idiom \idgloss{lo yesula be-paz}{will not be \textsc{sula} in gold}{priceless} where the cranberry verb \textnl{sula} is replaced with a \ili{Hebrew} synonym \hebgloss{he{\ayin}erix}{evaluate}. One such example is given in \reff{she:yesula-sub}.
...
Abbreviations used throughout examples in this chapter for agreement are: 1/2/3 = person; S/P = number; F/M = gender. In addition, \textsc{acc} = accusative case and CS = construct state.
