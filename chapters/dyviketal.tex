...
The verb has the semantic form \textsf{`tenke$\langle$[SUBJ]$\rangle$'}, meaning that the verb is intransitive and subcategorizes only for a subject, and the preposition has the semantic form `på$\langle$[OBJ]$\rangle$', indicating that it requires an object.
...
The simplified f-structure is shown in Figure~\ref{dyv:fig:mweiness:wws-fstr}; this is the ``\textsf{PRED}s only'' view of f-structure where feature paths that do not end in PRED values are suppressed. 
...
\todo[inline]{Please insert a subsection here}
...
The \isi{subcategorization} frame `klage\#nød$\langle$($\uparrow$~SUBJ)$\rangle$($\uparrow$~OBJ)' lists the semantic argument, in this case SUBJ, inside the angled brackets, while the selected argument OBJ is placed outside the brackets.
...
The \isi{template} invokes another \isi{template} CONCAT, which concatenates the predicate name P and the preposition form \emph{prp} as the value of the variable \%FN.
...
The \isi{phrase} \textit{et skarpt øye} fills the function of OBJ, but is not analyzed as a semantic argument of the sentence predicate, which appears from its position outside the angled brackets $\langle$...$\rangle$ surrounding the argument list.
...
\begin{table}
  \begin{tabular}{llll}
    ...
    V + PC & \emph{stå brud} & `stand bride' & `get married' \\ \tablevspace
